# My Perfect Days — Development specification

This document is the engineering source of truth: product rules, architecture, data models, security design, implementation history, roadmap, acceptance criteria, and validation targets. For the concise repository overview and local setup, see [README.md](README.md). Store packaging and release planning live in [DISTRIBUTION.md](DISTRIBUTION.md).
...
Editable reminder day/time/message fields now validate and commit on blur, preserving empty numeric drafts rather than coercing them to zero. Other settings number fields already commit on blur; onboarding numbers remain drafts until step confirmation and preserve an empty display. Check-in notes defer validation while focused. Switches and preset choices remain immediate actions. Valid reminder commits reconcile the updated schedule without duplicating unchanged notifications or reviving skipped/delivered occurrences. A localized scheduled date/time appears above the consistently styled skip action. About includes the React/TypeScript/Vite/Capacitor, local-storage, cryptography, translation and testing stack. Tests and builds are deferred for this UI change.
...
10. **Review before exclusion.** A surprising record remains part of the journal. Any decision not to use it in an estimate is visible, explained, reversible, and never presented as a diagnosis.
...
The first Phase 6 vertical slices are implemented as a deterministic, local **Cycle checks** system rather than opaque AI. It detects two high-confidence possible recording artifacts—two completed periods with known ends separated by no more than two clear days, and a cycle interval close to two or three times a sufficiently consistent personal baseline—plus a possibly forgotten end boundary on a long active period. An affected estimate interval is visibly held out until review; an active period is never ended automatically. The remaining rules in this section are planned. The system must never silently rewrite, merge, split, add, delete, or diagnose a recorded period.

### Finding categories

The product must keep three concepts separate:

1. **Data review:** a record may be incomplete, duplicated, split, or missing. These findings can affect whether a sample is temporarily used in an estimate.
2. **Personal pattern note:** a cycle differs from the user's established history but may be completely accurate. These findings normally reduce confidence or invite review rather than excluding data.
3. **Health guidance:** a recorded pattern may be worth discussing with a health professional. Health guidance never determines whether a journal entry is correct and never changes estimate inclusion.

The journal remains the source of truth in all three cases. A finding is not an error verdict, and a statistically unusual value is not proof of bad data. Clinical population ranges must not be reused as automatic data-cleaning thresholds.

### Initial rules

The initial thresholds below are versioned product hypotheses. They require synthetic edge-case tests, walk-forward evaluation, private-pilot review, and clinical-language review before public release.
...
| **Possible missing period** (implemented)   | With at least three other usable cycle intervals that form a consistent baseline, one interval is close to two or three times their leave-one-out personal median; consistency and multiple-match tolerance use the larger of three days or 15% of the relevant median | Temporarily quarantine that interval and offer to open History around the likely missing range                                                                            |
...
| **Period possibly left open** (implemented) | An active period exceeds the larger of eight days or the median of up to six recent reviewed known durations plus two days                                                                                                                                             | Ask whether the period is still active or its dates need review; never end it automatically and do not alter estimate samples                                             |
| **Possible recent pattern shift**           | Two or three consecutive recent cycles move consistently away from the older personal baseline                                                                                                                                                                         | Keep the new values, lower confidence, and describe a possible change instead of repeatedly calling them errors                                                           |
| **Highly variable usable history**          | Eligible recent cycles remain widely dispersed after review                                                                                                                                                                                                            | Keep an honest textual range and withhold calendar forecast coloring as in the current algorithm                                                                          |
| **Insufficient reviewed history**           | Review leaves too few eligible samples                                                                                                                                                                                                                                 | Use the optional fallback with rough confidence or withhold the estimate; never invent replacement samples                                                                |

Rule precedence must suppress duplicates: a likely missing-period finding takes precedence over a generic long-cycle outlier, and a possible split takes precedence over a generic short-cycle outlier. Start-only imported observations remain legitimate cycle-start evidence and remain excluded only from bleeding-duration calculations. Active periods remain excluded from cycle estimates under the existing rule.

Only high-confidence tracking-artifact rules may quarantine a sample by default, and that quarantine must be visible. Population-reference notices, pain, heavy flow, prolonged bleeding, and statistically unusual but plausible cycles are not automatic estimate exclusions. The current `heavy` and 1–5 pain fields do not contain enough information to infer urgent bleeding or functional impairment; any future safety prompt needs separately reviewed inputs and localized clinical wording.

### Estimate decisions and derivation

Generated findings remain derived and are recalculated after every relevant history edit. Only the user's explicit include/exclude decision or exact active-period acknowledgement is persisted. Schema version 5 added estimate decisions, while schema version 6 adds encrypted cycle-check acknowledgements:

```ts
type EstimateSampleKind = 'cycle' | 'duration';
type EstimateUse = 'include' | 'exclude';

type EstimateDecision = {
  sampleId: string;
  sampleKind: EstimateSampleKind;
  fingerprint: string; // affected episode IDs, dates, and update versions
  use: EstimateUse;
  reason: 'confirmed-correct' | 'recording-artifact' | 'missing-entry' | 'other';
  reviewedAt: string;
};

type CycleCheckAcknowledgement = {
...
The fingerprints prevent a decision about old boundaries from being applied after an affected record is changed. Stale decisions and acknowledgements are ignored safely and a later cleanup pass may prune them. The version-4-to-5 migration adds an empty `estimateDecisions` collection; the version-5-to-6 migration adds an empty `cycleCheckAcknowledgements` collection. Neither migration changes a recorded episode or log. Both collections live inside the vault, participate in encrypted backup/restore and readable export, and are removed by **Erase everything**; they must never be stored in URLs, browser history, Web Storage, caches, logs, or network requests.

The implemented possible-split and possible-missing slices use this pure domain pipeline:

```text
recorded episodes
  -> canonical cycle and duration samples
  -> deterministic cycle checks plus saved decisions
  -> one reviewed estimate dataset
...
Periods History shows a localized **Needs review** card above the period list for possible split and possible missing-period findings. Each finding names the affected dates, gives one short neutral explanation, and offers applicable implemented actions:

- **Review recorded dates**;
- **Add missing period**, which opens the existing History calendar near a derived likely range but does not create or persist a period;
- **Keep and use in estimates**;
- **Keep but exclude from estimates**; and
- leaving the finding unresolved without losing it.

A period possibly left open appears in the same review area after its personalized threshold. **Review period dates** opens the existing period controls, while **It is still active** persists an exact acknowledgement without changing the period. The acknowledgement stops the unchanged prompt from returning; editing that episode invalidates it, and completing the period removes the derived finding.
...
General clinical reference points—such as cycle frequency, bleeding duration, prolonged absence, or severe symptoms—belong in a separately reviewed health-guidance layer. Age, adolescence, perimenopause, postpartum state, and hormonal contraception can change what is expected; the first data-quality implementation should prefer personal history and data minimization rather than collecting a birth date or inferring a condition.
...
### Implemented foundation, secure core, Phase 5 mobile slice, and first Cycle checks slices
...
- A strict version-6 logical payload schema with sequential migrations, safe removal of legacy out-of-range forecast fallbacks, encrypted cycle-check acknowledgements, domain-invariant validation, and rejection of unsupported future versions.
...
- Canonical fingerprinted cycle and known-duration estimate samples, deterministic possible-split and possible-missing detection, default visible quarantine of affected intervals, and reversible **Keep and use**, **Keep but exclude**, and **Use again** decisions without changing recorded periods.
...
- [x] Implement the pure deterministic possible-missing-period rule only when at least three other usable intervals form a consistent personal baseline; quarantine the inflated interval without inventing a replacement period.
- [x] Detect a period possibly left active beyond the larger of eight days or the recent reviewed duration median plus two days; offer review or an exact persisted **Still active** acknowledgement and never end it automatically.
- [ ] Add isolated personal-outlier, unusual known-duration, and sustained-pattern-shift rules with explicit precedence and insufficient-history behavior.
...
### Phase 7 — Pilot and public-beta readiness

- Pilot privately over multiple real cycles.
- Test cycle-check terminology, review burden, false-positive findings, reversible exclusion, and forecast comprehension with menstruating users.
- Obtain clinical-language, privacy/legal, accessibility, and security review.
- Calibrate cycle-check thresholds and forecast ranges from held-out histories without using future information.
...
- [x] Possible missing-period review can open the existing History calendar near the relevant range without inventing or persisting a suggested period.
- [x] A long active period offers date review or a fingerprinted **Still active** acknowledgement without automatically ending or otherwise changing the episode.
- [ ] Repeated shifts are treated as possible new personal patterns with reduced confidence rather than repeatedly discarded as isolated errors.
- [ ] Too little reviewed history falls back honestly or withholds the estimate; it never manufactures replacement samples or certainty.
- [ ] Schema migration, encrypted backup/restore, readable export, erasure, cross-tab behavior, offline use, accessibility, and privacy-leakage tests cover persisted estimate decisions.
- [ ] Walk-forward evaluation proves that a rule sees only earlier training history and reports both prediction effects and review false positives before thresholds are described as validated.
- [ ] Health guidance is visually and architecturally separate from data review, does not alter estimates, and receives clinical and jurisdictional copy review before public release.
...
- [FIGO: Contemporary evaluation of women and girls with abnormal uterine bleeding](https://pmc.ncbi.nlm.nih.gov/articles/PMC10952771/)
- [JAMIA/PubMed: A predictive model for next cycle start date that accounts for adherence in menstrual self-tracking](https://pubmed.ncbi.nlm.nih.gov/34534312/)
